...
const ProductCard = ({data}) => {
  return (
    <div className="max-w-sm bg-white rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 w-full mx-auto ">
      <a href="#">
        <img className="p-8 rounded-t-lg hover:scale-110" src={data?.img} alt="product image" />
      </a>
      <div className="px-5 pb-5">
          <h5 className="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white mb-2  hover:before:content-['--'] transition duration-700 ease-in-out">{data?.name}</h5>
          <h5 className="text-lg italic font-semibold tracking-tight text-gray-900 dark:text-white mb-2 ">{data?.body}</h5>
          <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white hover:before:content-['--'] duration-1000">Supplier: {data?.supplier}</h5>
          <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white mb-2 hover:before:content-['--'] duration-1000">Quantity: {data?.quantity}</h5>
